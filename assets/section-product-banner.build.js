(function () {
     ...
     * ------------------------------------------------------------------------------
     * @namespace ProductBanner
     */

    const selectors = {
    	section: ".js-product-banner",
    	productsSlider: ".js-product-banner-slider",
    	thumbsSlider: ".js-product-banner-thumbs-slider",
    	thumbsSlide: ".js-product-banner-thumbs-slide",
    	sliderUI: ".js-product-banner-slider-ui",
    ...
    var ProductBanner = () => {
    	...
    	async function init(sectionId) {
    		...
    		Swiper.use([EffectFade, Pagination, Controller]);
    		...
    		if (sections) {
    			sections.forEach((section) => {
    				sectionComponents.push({
    					...
    					thumbsSlider: section.querySelector(selectors.thumbsSlider),
    					sliderUI: section.querySelector(selectors.sliderUI),
    	...
    	function slidersInit() {
    		sectionComponents.forEach((section) => {
    			...
    			if (slider.dataset.slidesCount && parseInt(slider.dataset.slidesCount) <= 1) {
    				return;
    			}

    			const thumbsSlider = section.thumbsSlider;
    			const sliderUI = section.sliderUI;
    			...
    			Thumbnails = thumbsSliderInit(thumbsSlider);
    			MainSlider = sliderInit(slider, buttonNext, buttonPrev, pagination, sliderUI);

    			MainSlider.controller.control = Thumbnails;
    			Thumbnails.controller.control = MainSlider;

    			Thumbnails.on("slideChange", function () {
    				playCurrentVideo(thumbsSlider, Thumbnails);
    			});

    			on("change", mediaQueryList, () => {
    				playCurrentVideo(thumbsSlider, Thumbnails);
    			});
    		});
    	}

    	function sliderInit(slider, buttonNext, buttonPrev, pagination, sliderUI) {
    		return new Swiper(slider, {
    			...
    			on: {
    				init: function () {
    					if (sliderUI) {
    						sliderUI.classList.add("initialized");
    					}
    				}
    			}
    		});
    	...
    	function playCurrentVideo(slider, Swiper) {
    		Swiper.slides.forEach((slide) => {
    			if (slide.querySelector(selectors.video)) {
    				slide.querySelector(selectors.video).pause();
    			}
    		});

    		const currentVideo = [...slider.querySelectorAll(`${selectors.thumbsSlide}:nth-child(${Swiper.activeIndex + 1}) ${selectors.video}`)].find(
    	...
    	function thumbsSliderInit(thumbsSlider) {
    		return new Swiper(thumbsSlider, {
    			...
    			on: {
    				init: function (Swiper) {
    					const currentVideo = [...thumbsSlider.querySelectorAll(`${selectors.thumbsSlide}:nth-child(${Swiper.activeIndex + 1}) ${selectors.video}`)].find(
    						(video) => getComputedStyle(video).getPropertyValue("display") !== "none"
    					);
